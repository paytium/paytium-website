...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const image = article.image ? [{ url: article.image, alt: article.imageAlt.fr }] : [];
  return { title: `Paytium | ${article.title.fr}`, description: article.summary.fr, alternates: { canonical, languages: { "fr-FR": canonical, "en-US": `/en/blog/${slug}/` } }, openGraph: { title: article.title.fr, description: article.summary.fr, url: canonical, type: "article", locale: "fr_FR", alternateLocale: ["en_US"], publishedTime: article.date, authors: ["Paytium"], tags: [article.theme.fr], images }, twitter: { card: article.image ? "summary_large_image" : "summary", title: article.title.fr, description: article.summary.fr, images: image } };
}
