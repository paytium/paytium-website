  ...
  <PageShell locale={locale} translationHref={blogHref(article, locale === "fr" ? "en" : "fr")} activeNav="blog">
    ...
    <article className="blog-article">
      ...
      <div className="article-content">{article.sections[locale].map((block, blockIndex) => {
        ...
        return <div className="article-sources" key={blockIndex}><h2>{locale === "fr" ? "Sources et portée de l’analyse" : "Sources and scope of analysis"}</h2><ul>{block.items.map((item) => <li key={item.url}><a href={item.url} target="_blank" rel="noreferrer">{item.label}</a></li>)}</ul></div>;
      })}</div>
    </article>
