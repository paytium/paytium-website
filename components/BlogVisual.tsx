...
export function BlogVisual({ article, index = 0, large = false, locale = "fr" }: { article: BlogArticle; index?: number; large?: boolean; locale?: BlogLocale }) {
  if (article.image) return <div className={`blog-visual ${large ? "large" : ""}`}><img src={article.image} alt={article.imageAlt[locale]} /><span className="blog-visual-brand" aria-hidden="true"><b>PAYTIUM</b> INSIGHTS</span></div>;
