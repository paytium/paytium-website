...
export function TechPreview({ locale = "fr" }: { locale?: "fr" | "en" }) {
  ...
  return <div className="tech-preview">{technologies.map((group, index) => <article key={group.title}><span aria-hidden="true">{symbols[index % symbols.length]}</span><h3>{locale === "en" ? englishTitles[group.title] ?? group.title : group.title}</h3><p>{group.items.join(" · ")}</p></article>)}</div>;
}
