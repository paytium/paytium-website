...
function HeroVisual({ type }: { type: string }) {
  if (type === "invoice") return <div className="hero-diagram invoice-diagram"><div className="orbital-lines"/><div className="flow-node"><small>SOURCE</small><b>ERP</b></div><div className="flow-hub"><img src="/favicon.svg" alt=""/><span>Paytium</span></div><div className="flow-node end"><small>ÉCHANGE</small><b>Partenaires</b></div><div className="status-stack"><span>✓ Contrôlée</span><span>↗ Transmise</span><span>□ Archivée</span></div></div>;
  ...
  return <div className="hero-diagram trajectory-diagram"><div className="orbital-lines"/><div className="paytium-core"><img src="/favicon.svg" alt=""/></div><div className="float-label label-a">Stratégie</div><div className="float-label label-b">Produit</div><div className="float-label label-c">Data</div><div className="float-label label-d">Cloud</div><div className="trajectory-card"><small>TRAJECTOIRE</small><b>Vision → Produit → Impact</b><i><span/></i></div></div>;
}
...
export function HomeHero() {
  ...
  return (
    <section className="home-hero" aria-roledescription="carousel" aria-label="À la une" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} onFocusCapture={() => setPaused(true)} onBlurCapture={() => setPaused(false)} onKeyDown={(event) => { if (event.key === "ArrowLeft") go(active - 1); if (event.key === "ArrowRight") go(active + 1); }}>
      <div className="hero-slide" key={active} aria-live="polite">
