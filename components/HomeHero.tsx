...
import { LuArrowLeft, LuArrowRight, LuBadgeCheck, LuBriefcaseBusiness, LuCloudCog, LuCode, LuDatabase, LuGauge, LuRefreshCw, LuShieldCheck, LuSparkles, LuTestTube, LuUsers } from "react-icons/lu";
...
const slidesFr = [
  {
    ...
    text: "Notre Digital & Data Factory transforme votre backlog en incréments utiles : cadrage partagé, sprints de build et de test, revues régulières, mise en production, puis TMA, centre de services ou transfert de compétences.",
...
const slidesEn = [
  { eyebrow: "DIGITAL & DATA FACTORY", title: <>From framing to run, deliver through <em>short, controlled cycles.</em></>, text: "Our Digital & Data Factory turns your backlog into valuable increments through shared framing, build-and-test sprints, regular reviews, production releases, then application support, managed services or capability transfer.", primary: ["Explore the Digital & Data Factory", "/en/services/#digital-data"], secondary: ["Talk to an expert", "/en/#contact"], visual: "agile" },
...
function HeroVisual({ type, locale }: { type: string; locale: "fr" | "en" }) {
  ...
  if (type === "talent") return <div className="hero-diagram talent-diagram"><div className="engineering-heroes"><div className="heroes-heading"><LuSparkles aria-hidden="true"/><span><small>PAYTIUM HERO SQUAD</small><b>{locale === "fr" ? "Une équipe. Toutes les expertises." : "One team. Every capability."}</b></span></div><div className="hero-team"><div className="hero-profile hero-product"><i><LuBriefcaseBusiness/></i><b>Product</b><small>PO · UX</small></div><div className="hero-profile hero-software"><i><LuCode/></i><b>Software</b><small>Web · Mobile · API</small></div><div className="hero-profile hero-data"><i><LuDatabase/></i><b>Data & AI</b><small>Data · ML · GenAI</small></div><div className="hero-profile hero-cloud"><i><LuCloudCog/></i><b>Cloud</b><small>DevSecOps · SRE</small></div><div className="hero-profile hero-quality"><i><LuBadgeCheck/></i><b>Quality</b><small>QA · Agile</small></div></div><div className="hero-delivery-chain"><span>{locale === "fr" ? "Vision" : "Vision"}</span><i/><span>Build</span><i/><span>Secure</span><i/><span>Scale</span></div><div className="hero-seniority"><LuUsers aria-hidden="true"/><span>Junior</span><i/><span>{locale === "fr" ? "Confirmé" : "Confirmed"}</span><i/><span>Senior</span><i/><span>Expert</span></div></div></div>;
  return <div className="hero-diagram agile-delivery-diagram"><div className="agile-loop" aria-hidden="true"><span/><span/><span/></div><div className="agile-core"><img src="/paytium-icon-white.svg" alt=""/><small>DAILY SCRUM</small><b>BUILD & TEST</b></div><div className="agile-stage agile-scope"><small>01</small><b>{locale === "fr" ? "Cadrage" : "Framing"}</b><span>Vision · Backlog</span></div><div className="agile-stage agile-build"><small>02</small><b>Sprint Planning</b><span>{locale === "fr" ? "Priorités partagées" : "Shared priorities"}</span></div><div className="agile-stage agile-release"><small>03</small><b>Sprint Review</b><span>{locale === "fr" ? "Démonstration · Feedback" : "Demo · Feedback"}</span></div><div className="agile-stage agile-run"><small>04A</small><b>{locale === "fr" ? "TMA / Centre de services" : "Support / Managed services"}</b><span>{locale === "fr" ? "Évolution continue" : "Continuous evolution"}</span></div><div className="agile-stage agile-transfer"><small>04B</small><b>{locale === "fr" ? "Transfert de compétences" : "Capability transfer"}</b><span>{locale === "fr" ? "Autonomie client" : "Client autonomy"}</span></div><div className="agile-feedback"><LuRefreshCw aria-hidden="true"/>{locale === "fr" ? "Rétrospective · amélioration continue" : "Retrospective · continuous improvement"}</div></div>;
}
