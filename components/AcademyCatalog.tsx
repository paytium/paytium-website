...
const labels = {
  ...
  en: { allDomains: "All domains", allTypes: "All types", allFormats: "All formats", search: "Search for a course", placeholder: "E.g. AI, Product, Kubernetes…", results: "courses found", empty: "No course matches these criteria.", reset: "Reset filters", duration: "Duration", format: "Formats", details: "Request the programme", types: { Technology: "Technology", Methods: "Methods", "Responsible Digital": "Responsible digital" }, formats: { "Inter-company": "Inter-company", "In-company": "In-company", Custom: "Custom" } },
} as const;
...
export function AcademyCatalog({ locale = "fr" }: { locale?: AcademyLocale }) {
  ...
  return <div className="academy-catalog">
    <div className="catalog-filters" role="search">
      <label className="catalog-search"><span>{copy.search}</span><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={copy.placeholder} /></label>
    ...
    {courses.length ? <div className="course-grid">{courses.map((course) => <article className="course-card" key={course.code}>
      ...
      <a href={`${locale === "en" ? "/en" : ""}/#contact`}>{copy.details}<span aria-hidden="true">↗</span></a>
